...
import MonsterGrowthRulesModal from './MonsterGrowthRulesModal';
...
const MonsterListComponent: React.FC = () => {
    ...
    const getImageUrl = (monsterType: "Normal" | "Premium" | "Cat" | "Flame" | "Ghost" | "Dog" | "Dinosaur" | "Metal", growPoints: number) => {
        const base = monsterType ? `/images/${monsterType}/` : '/images/Normal/';
        const stage = 
            growPoints < 100 ? 0 :
            growPoints < 200 ? 1 :
            growPoints < 300 ? 2 :
    ...
    const renderMonsterCard = (monster: Monster | null, monsterType: "Normal" | "Premium" | "Cat" | "Flame" | "Ghost" | "Dog" | "Dinosaur" | "Metal") => {
        ...
        return (
            <div className="relative bg-white p-2 rounded-lg shadow-sm text-center">
                ...
                {monsterType !== 'Normal' && (
                    <span className="absolute top-1 right-1 text-[8px] bg-gradient-to-r from-yellow-400 to-yellow-600 text-black px-1 py-0.25 rounded-full font-bold">
                        Premium+
                    </span>
                )}
                <div className="mt-2">
    ...
    return (
        <div className="max-w-5xl mx-auto mt-4 p-4 bg-yellow-50 rounded-lg shadow-md text-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-4">
                ...
                {renderMonsterCard(monsterRes.flame_monster, 'Flame')}
                {renderMonsterCard(monsterRes.ghost_monster, 'Ghost')}
                {renderMonsterCard(monsterRes.dog_monster, 'Dog')}
                {renderMonsterCard(monsterRes.dinosaur_monster, 'Dinosaur')}
                {renderMonsterCard(monsterRes.metal_monster, 'Metal')}
                {/* Coming Soon Section */}
