...
class GetMonsterAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            monster = Monster.objects.filter(account=account.id,monster_type=monster_selected.selected_monster).first()
            if monster is None:
                return Response({'message': 'No Monster found'}, status=status.HTTP_404_NOT_FOUND)
            serialized_monster = MonsterSerializer(monster)
            
            return Response({'message': 'Get monster successfully!', 'data': {'monster':serialized_monster.data, 'selected_stage': monster_selected.selected_stage}}, status=status.HTTP_200_OK)
